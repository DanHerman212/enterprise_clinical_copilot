...
recorded as a decision rather than left unresolved. Four of the five entries are closed by
implementation. The fifth — the retention policy — is decided and half implemented: the store
enforces the window and the ceiling, a sweep command exists with a dry run, and what is *not*
yet true is that anything runs that sweep in production, and that the two other holdings of
clinical text have the lifetimes they were given. Both are named in the entry rather than left
implied, because a retention promise that exists only in code is not a retention promise.
