...
recorded as a decision rather than left unresolved. All five entries are closed by
implementation, including the retention policy: the store enforces the window and the ceiling, a
sweep command exists, a Cloud Run job runs it in production on a daily schedule, and the two
other holdings of clinical text have stated lifetimes that something enforces.
...
| 3 | No retention policy exists for a conversation, and three holdings of clinical text already exist without one: the evaluation harness's trace archive, the retrieval layer's note cache, and the captured fixture payloads committed to both repositories (`evaluation/agent/collect.py` 1–8). The answer payload carries passage text per citation, so a store of "the citations that were shown" holds note text unless the stored shape drops that field (`services/agent/contracts.py` 23–27). | Fix the policy before the store, and reconcile it with the three holdings rather than writing it around them: what is kept, for how long, who may read it, what an account deletion or the end of the demonstration does to it, and what the logs hold as distinct from what the product holds. Name the stored shape explicitly, including which fields of a citation are kept. | Open. Policy decision required; it precedes the store because the store is built to it. Decided 2026-09-18: **twenty-four hours, swept by a job rather than by lazy expiry** — a row that is never read is never swept by a reader, and PostgreSQL has no native row expiry — **a stored citation keeps its identity and not its passage text, and each of the three existing holdings is given its own stated lifetime.** **Decided 2026-09-18: the store keeps tool names, their arguments and the payloads that cannot be re-derived — the prediction scores — and retrieval payloads are re-derived on demand, so no clinical note text enters the session store. The evaluation trace archive gets a fixed longer lifetime for auditing, the note cache is treated as ephemeral and regenerable with a very short lifetime, and the committed fixtures carry no runtime lifetime at all: they are a repository decision, kept synthetic or de-identified.** **Implemented 2026-09-18: the store enforces the window and the ceiling from settings, the sweep is a management command with a dry run, and ten tests hold the policy. **Closed 2026-09-18.** The sweep runs in production now: `purge-conversations` is a Cloud Run job defined in the site's `cloudbuild.yaml` and attached to Cloud SQL, and Cloud Scheduler's `purge-conversations-daily` executes it at 03:00 America/New_York with three retries. Both halves were verified by running them rather than by reading them — the job printed "no expired conversations" against the production database, and a forced scheduled attempt produced a completed execution. The other two holdings have mechanisms behind their lifetimes (`scripts/agent/retention.py`, a dry run unless `--apply` is passed): the note cache is deleted in full because it is regenerable, and the trace archive is pruned to a 90-day window. Two things this clause got wrong, corrected rather than left standing. The archive is not in a bucket — it is `evaluation/agent/results/traces.jsonl` on the machine that runs an evaluation, so there was never a lifecycle rule to add; and its records carried no timestamp, so no window could be applied to them at all until `collect.py` began writing one. A record predating that field is kept and counted, because an undated record cannot be aged and deleting it would be a decision nobody made. Enforcing the archive's window also found that `collect.py` had been computing its output path from a `parents[1]` that resolved to `evaluation/eval/results/` — a path that does not exist and that `.gitignore` does not cover, so a run would have left clinical text untracked but committable. It now writes beside the golden sample it reads, and a test asserts the writer and the pruner name the same file.** |
