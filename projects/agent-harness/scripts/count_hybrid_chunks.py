...
NOTE: pack_to must match the pipeline default (700, not 1). The pipeline packs
sections into ~700-char chunks; counting with pack_to=1 overcounts (307 vs the
actual 136) and makes the build-index gate fail.
"""
...
def main() -> int:
    ...
    for row in client.query(sql).result():
        ...
        for chunk in chunk_note(note, max_chars=DEFAULT_MAX_CHARS, pack_to=PACK_TO):
            ...
            total += 1
    print(f"pack_to={PACK_TO} TOTAL chunks: {total}")
