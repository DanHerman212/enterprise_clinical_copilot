...
SCRIPTS = REPO_ROOT / "scripts" / "agent"
...
def _cloudbuild_text(name: str) -> str:
    path = {
        "cloudbuild.agent.yaml": REPO_ROOT / "services" / "agent" / "cloudbuild.yaml",
        "cloudbuild.mcp.yaml": REPO_ROOT / "services" / "mcp" / "cloudbuild.yaml",
    }[name]
