...
Pins:
  ...
  * ECC-29: services.mcp.config validates FEATURE_TABLE / DISCHARGE_TABLE env
...
from services.mcp.config import _validated_table_ref as config_validate

# The MLOps and retrieval pipelines use different component package names, so
# the retrieval component is loaded by file path under a synthetic package
# (the relative `._image` import needs a parent package with a __path__).
_COMP_DIR = (
    Path(__file__).resolve().parents[2]
    / "services" / "mcp" / "pipelines" / "components"
)
